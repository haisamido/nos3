...
do
    ...
    gnome-terminal --tab --title=$SC_NUM" - Torquer Sim"     -- $DFLAGS -v $SIM_DIR:$SIM_DIR -v /dev/shm:/dev/shm --name $SC_NUM"_torquer_sim"     --network=$SC_NETNAME -w $SIM_BIN $DBOX ./nos3-single-simulator $SC_CFG_FILE generic_torquer_sim
